...
var ComponentGenerator = yeoman.generators.NamedBase.extend({
  init: function() {
    ...
    try {
      this.libraryName = require(path.join(process.cwd(), 'package.json')).name;
    } catch (e) {
  ...
  files: function() {
    this.componentSrcDir = 'src/app/' + this.componentId;
    this.componentTplDir = this.componentSrcDir;
    //Src directory
    this.mkdir(this.componentSrcDir);
    
    //this.mkdir(this.componentSrcDir + '/test')
    this.template('_component_src/test/_component.spec.js', this.componentSrcDir + '/' + this.componentId + '.spec.js')
